...
class SetupModel:
    ...
    def _update_emme_project_for_dual_databases(self):
        """
        Update EMME project file (.emp) to fix case mismatches in database paths.
        
        The template .emp file may have inconsistent casing in ProjectDatabases entries
        (e.g. 'database_highway' vs 'Database_highway' on disk). This method normalizes
        them to match the actual directory names on disk.
        
        Note: OpenDatabases does NOT support comma-separated values. Multiple databases
        are opened at runtime via the EMME API (EmmeManager.add_database()) instead.
        """
        emp_files = list((self.model_dir / "emme_project").glob("*.emp"))
        if not emp_files:
            self.logger.warning("No EMME project file (.emp) found - skipping database configuration")
            return
        
        emp_file = emp_files[0]  # Use first .emp file found
        emme_project_dir = self.model_dir / "emme_project"
        self.logger.info(f"Checking EMME project file for database path issues: {emp_file}")
        ...
        try:
            ...
            # Fix case mismatches in ProjectDatabases entries
            # The template may have e.g. 'database_highway\emmebank' but the actual
            # directory on disk is 'Database_highway'
            def fix_project_database_case(match):
                original_path = match.group(1)  # e.g. 'database_highway\emmebank'
                rest = match.group(2)  # e.g. ';;windows-1252'
                
                # Extract the directory name (before \emmebank or /emmebank)
                parts = re.split(r'[/\\]', original_path)
                db_dir_name = parts[0]
                
                # Check if the actual directory on disk has different casing
                actual_dirs = {d.name: d.name for d in emme_project_dir.iterdir() if d.is_dir()}
                actual_dirs_lower = {d.lower(): d for d in actual_dirs}
                
                if db_dir_name not in actual_dirs and db_dir_name.lower() in actual_dirs_lower:
                    correct_name = actual_dirs_lower[db_dir_name.lower()]
                    fixed_path = original_path.replace(db_dir_name, correct_name, 1)
                    self.logger.info(f"Fixed ProjectDatabases case mismatch: '{original_path}' -> '{fixed_path}'")
                    return f"ProjectDatabases[] = {fixed_path}{rest}"
                
                return match.group(0)
            
            updated_content = re.sub(
                r'ProjectDatabases\[\] = ([^;\n]+)(;;[^\n]*)',
                fix_project_database_case,
                updated_content
            )
            ...
            if updated_content != content:
                ...
                self.logger.info("Updated EMME project file to fix database path issues")
            else:
                self.logger.info("EMME project file database paths are correct")
                
        except Exception as e:
            self.logger.error(f"Failed to update EMME project file: {e}")
