...
class SetupModel:
    ...
    def run_setup(self):
        ...
        folders_to_create = [
            'acceptance', 'CTRAMP', 'ctramp_output', 'demand_matrices', 'emme_project', 'inputs', 'logs', 'warmstart',
            'notebooks', 'output_summaries', 'skim_matrices', 'skim_matrices/highway', 'skim_matrices/transit', 'skim_matrices/non_motorized'
        ]
        ...
        config_files_list = ["observed_data.toml", "canonical_crosswalk.toml", "model_config.toml", "scenario_config.toml"]
        acceptance_config_files_list = ["observed_data.toml", "canonical_crosswalk.toml"]

        for file in config_files_list:
            github_url = os.path.join(configs.CONFIGS_GITHUB_PATH, file)
            github_url = github_url.replace("\\", "/")

            local_file = os.path.join(model_dir, "acceptance" if file in acceptance_config_files_list else "", file)
        ...
        self._download_github_release(org, repo, tag, folders_to_extract, os.path.join(model_dir, "CTRAMP"), logger)

        # Rename 'uec' folder to 'model'
        os.rename(os.path.join(model_dir, "CTRAMP", "uec"), os.path.join(model_dir, "CTRAMP", "model"))
    ...
    def _copy_folder(self, src_dir, dest_dir, logger):
        ...
        if os.path.exists(src_dir):
            ...
            raise FileNotFoundError(f"Source directory {src_dir} to copy from does not exists.")
    ...
    def _download_file_from_github(self, github_url, local_file, logger):
        ...
        try:
            ...
            raise Exception(f"Failed to download file {github_url} from GitHub: {str(e)}")
    
    
    def _download_github_release(self, org_name, repo_name, release_tag, folders_to_extract, local_dir, logger):
        """
        ...
        try:
            ...
            for file_info in z.infolist():
                if not file_info.is_dir():
                    ...
                    if any(file_path.startswith(folder) for folder in folders_to_extract):
                        ...
                        # extract the file
                        with z.open(file_info.filename) as source, open(extract_path, 'wb') as target:
                            target.write(source.read())
    
                        copied_folder.add(file_path.split('/')[0])
                        
            if copied_folder is not None:
                logger.info(f"Extracted folders {copied_folder} from GitHub release {release_url} and to directory {local_dir}")
        
        except Exception as e:
            logger.error(f"Failed to download GitHub release {release_url}: {str(e)}")
            raise Exception(f"Failed to download GitHub release {release_url}: {str(e)}")
    ...
    def _copy_model_inputs(self, configs, model_dir, logger):
        ...
        self._copy_folder(os.path.join(configs.INPUT_NETWORK_DIR, "hwy"), os.path.join(model_dir, "inputs", "hwy"), logger)
        self._copy_folder(os.path.join(configs.INPUT_NETWORK_DIR, "trn"), os.path.join(model_dir, "inputs", "trn"), logger)
    
        # Copy popsyn and landuse inputs
        self._copy_folder(os.path.join(configs.INPUT_POPLU_DIR, "popsyn"), os.path.join(model_dir, "inputs", "popsyn"), logger)
        self._copy_folder(os.path.join(configs.INPUT_POPLU_DIR, "landuse"), os.path.join(model_dir, "inputs", "landuse"), logger)
    
        # Copy nonres inputs
        self._copy_folder(os.path.join(configs.INPUT_NONRES_DIR, "nonres"), os.path.join(model_dir, "inputs", "nonres"), logger)
    
        # Copy warmstart demand 
        self._copy_folder(configs.WARMSTART_DEMAND_DIR, os.path.join(model_dir, "warmstart"), logger)
    ...
    def _copy_emme_project_and_database(self, configs, model_dir, logger):
        ...
        self._copy_folder(configs.EMME_TEMPLATE_PROJECT_DIR, os.path.join(model_dir, "emme_project"), logger)
    
        # copy emme network database
        self._copy_folder(os.path.join(configs.INPUT_EMME_NETWORK_DIR, "emme_drive_network", "Database"), os.path.join(model_dir, "emme_project", "Database_highway"), logger)
        self._copy_folder(os.path.join(configs.INPUT_EMME_NETWORK_DIR, "emme_taz_transit_network", "Database"), os.path.join(model_dir, "emme_project", "Database_transit"), logger)
        self._copy_folder(os.path.join(configs.INPUT_EMME_NETWORK_DIR, "emme_maz_active_modes_network_subregion_north", "Database"), os.path.join(model_dir, "emme_project", "Database_active_north"), logger)
        self._copy_folder(os.path.join(configs.INPUT_EMME_NETWORK_DIR, "emme_maz_active_modes_network_subregion_south", "Database"), os.path.join(model_dir, "emme_project", "Database_active_south"), logger)
...
class Config:
    ...
    def validate(self):
        ...
        required_attrs = [
            'ROOT_DIR', 'MODEL_FOLDER_NAME', 'INPUT_NETWORK_DIR', 'INPUT_POPLU_DIR',
            'WARMSTART_DEMAND_DIR', 'CONFIGS_GITHUB_PATH', 'TRAVEL_MODEL_TWO_RELEASE_TAG'
        ]
