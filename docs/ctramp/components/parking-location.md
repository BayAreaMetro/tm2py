...
Parking inputs include both [hourly, daily, monthly] stalls in same MAZ and in different MAZs. While these variables are both used in the UEC and choice model, it serves more as way to note if the MAZ considered as an alternative is the same MAZ as the destination MAZ. The values for number of stalls in same MAZ and in different MAZ are the same in the MAZ input files.  For more details on parking input creation, see the [land use input documentation in tm2py-utils](https://github.com/BayAreaMetro/tm2py-utils/tree/main/tm2py_utils/inputs/land_use).